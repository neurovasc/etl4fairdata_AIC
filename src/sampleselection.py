...
agp.add_argument('-o', '--outsamples', type=str, help='Path to the output file (list of samples)', required=True)
agp.add_argument('-f', '--outfile', type=str, help='Path to the output file (filtered clinical csv)', required=True)
...
try:
    ...
    for index, row in filtered_clinical.iterrows():
        if row['N°ADN IRT 1'] not in merged_samples and row['N°ADN IRT 1'] not in aic_samples:
            ...
            clinical_samples.append(row)
    # Write aic_samples to outfile parameter pat
    with open(args.outsamples, 'w') as f: f.write('\n'.join(aic_samples))
    with open(args.outsamples, 'w') as f: f.write('\n'.join(clinical_samples))
except:
    pass 
# For simulated data
try:
